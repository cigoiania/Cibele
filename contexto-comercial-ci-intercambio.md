...
> unidades. Central de vendas citada: (11) 4560-8440. ⚠️ A ESCLARECER: escopo
> real (só intercâmbio? turismo também?).
...
qualificação inicial**: acolher o lead, coletar os dados essenciais, tirar
dúvidas básicas e **encaminhar para uma reunião com o consultor**. A Cibele
qualifica e agenda; **quem apresenta proposta e fecha é o consultor**.

Fluxo padrão do primeiro atendimento (ensinado por Ailton):
1. Acolher e identificar o que o lead busca (ex.: intercâmbio para a filha).
2. Qualificar (ver Seção 6).
...
4. **Próximo passo padrão** se mantém: alinhar o interesse → encaminhar ao
   consultor (agendamento). Mensagens prontas no **Modelo E** (Seção 7).
...
> de **Trabalhar e Estudar na Espanha**. Ver **Modelo E** (Seção 7).

## 6. Qualificação do Lead

Perguntas de qualificação no primeiro atendimento (exemplo: mãe buscando
intercâmbio para a filha):

1. **Nível de inglês** do aluno(a).
2. **Destino** e, conforme o país, situação de **visto**:
   - **EUA:** perguntar se já tem o visto americano.
   - **Canadá:** perguntar sobre o visto; **o visto americano também "vale"
     como facilitador** — ter visto americano facilita muito a obtenção do
     visto canadense.
...
8. **Objetivo principal.** Ex.: "desenvolvimento profissional" → investigar a
   **área de atuação** do lead (em que trabalha / quer se desenvolver), além do
   nível de inglês.
9. **Idioma.** Em geral é **definido pelo país de destino** (Austrália → inglês;
   **Espanha → espanhol**). ⚠️ Se algum campo do cadastro trouxer um idioma que
   **não bate com o destino** (ex.: "inglês" para a Espanha), o campo está
   provavelmente **errado** — **vale o idioma do país**. Se o lead não confirmou
   diretamente, tratar como suposição e validar.
...
**Modelo E — Lead reativado (abordagem de retorno)**
...
- Mensagens curtas, humanas e calorosas; tom acolhedor, brasileiro, uso leve de
  emojis.
...
- **2026-07-09** — Primeiro treinamento (Ailton): fluxo do primeiro atendimento
  como triagem/qualificação → reunião com consultor. Registrado nas Seções 4
...
- **2026-07-23** — Simulação + treinamento (Marcelo): abordagem para **lead
  ...
  desativação, revalidar interesse/dados antes de qualificar) e **Modelo E** de
  mensagem (Etapas 1 e 2) na Seção 7. Correções/aprendizados confirmados pelo
